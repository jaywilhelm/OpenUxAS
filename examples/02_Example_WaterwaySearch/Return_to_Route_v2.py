...
from uavData import uavData, UAV_TYPE, UAV_AVOID
import AvoidanceLogic 
...
uavtype = UAV_TYPE.DUBINS
uav2 = uavData(uavinfo, uavtype, UAV_AVOID.INACTIVE)
uavlist = []
uavlist.append(uav1)
uavlist.append(uav2)
...
while step <= 900:
    ...
    otherUAVs = R2R.findotheruavs(uavlist, 1)       # ID not to watch for
    ...
    #         TargetWPList.append(ActivePath_List[i]['pt'])clear
    ...
    if not hasAstarPlan and Test == True:
        ...
        if detect == True:
            ...
            if (astarReplan and not hasAstarPlan):
                ...
                indexRecall = mainUAV.getActiveWaypointIndex()-1 # used to inform Astar goal and Revocery path logic where the UAV first left the reference path

                # Current positions is saved to be used later as a possible recovery insetions point
                saveCurrentPOS = mainUAV.getPosition()

                ''' Insert current vehicle postion and astar goal into A* replan '''
                astarwpts[0][0] = saveCurrentPOS[0]
                astarwpts[0][1] = saveCurrentPOS[1]
                ...
                Astar_Index2Watch4 = mainUAV.getActiveWaypointIndex() + numbOfAstarPts
                if lap == True:
    ...
    if hasAstarPlan and ClearedCollision == True:
        ...
        # RecoveryPoints, referencePathPoint, InsertionPoint_list = 
        R2R.getPotentialRecoveryPoints(ActivePath_List, ReferencePath_List, saveCurrentPOS, indexRecall, numbOfPts=5)
...
#         RecoveryPathFound, chosenClothoid, acceptedPaths, selectIndex, lap = 
        R2R.selectRecoveryPath(clothoid_List, index_List, indexRecall, halfTurnRadius, 10, lap)

        R2R.RecoveryPaths_SnapShot('/RaceTrack_RecoveryPaths', step, clothoid_List, 
                        acceptedPaths, PruitTrack, astarwpts, KOZpoints, False, RecoveryPoints, 
                        activeWP, mainUAV, uavh_others_all, area_length, fig, ax)
        ...
        if RecoveryPathFound:
            ...
            mainUAV.setWaypoints(TargetWPList, newradius=wptRad/2)
            mainUAV.setCurrentWaypointIndex(Astar_Index2Watch4)
            activeWP = Astar_Index2Watch4 #mainUAV['dubins'].waypoints[mainUAV['dubins'].currentWPIndex]

            R2R.RecoveryPath_SnapShot('/RaceTrack_SelectedPaths', step, PruitTrack, astarwpts, 
            pltPts_List, RecoveryPathWPs, activeWP, mainUAV, uavh_others_all, area_length, fig, ax )
    ...
    for uavx in uavlist:
        if(uavx.uavType == UAV_TYPE.DUBINS):
            uavx.update(None)
        elif (uavx.uavType == UAV_TYPE.MAVLINK):
            error('no mavlink supported')
            # messages = position + heading + other?
            # uavx.update(messages)

        elif (uavx.uavType == UAV_TYPE.CMASI):
            # messages = airvehicle state? + waypoint list?
            # uavx.update(messages)
            error('no CMASI supported')
        else:
            error('i broke it')

    'Plotting stuff'
    # ??
    CASCone_pts = R2R.flightProjection(mainUAV.position[0], mainUAV.position[1], mainUAV.heading, mainUAV.velocity, mainUAV.data['turnRate'], dt, lookAhead_time)
    color = '-g'
    if(astarReplan):
        color = '-y'
        replan = False
    ...
    print('\nStep: ' + str(step) + '\tUAV ID: ' + str(mainUAV.data['ID']) + '\tPosition: ' + str(round(mainUAV.position[1])) +' ' + str(round(mainUAV.position[0])) + ' Heading: ' + str(round(np.rad2deg(mainUAV.heading),3)) + '\tWpt: ' + str(mainUAV.data['current_waypoint']))
    #print('\nStep: ' + str(step) + '\tUAV ID: ' + str(mainUAV.data['ID']) + '\tPosition(dubins):' + '\t' + str(mainUAV.uavDubins.position))
