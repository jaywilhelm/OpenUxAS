...
class UAVHeading:
    ...
    '''
    def avoid(self, uavh_others, turnRate, dt, lookAhead_time, area_length, static_koz, TargetPathWP, useAstarGoal, simStep=0):
        ...
        intersects, avoid_areas, CollisionUavIDs = self.NewfindPotentialIntersects(uavh_others, area_length, static_koz, turnRate, dt, lookAhead_time) # Browne Nov 2020
        ...
        # # ax.plot(t, s)
        # ax1.plot(start[1], start[0], '*', c='r', markersize=8)
        # ax1.plot(goal[1], goal[0], 'X', c='g', markersize=8)
        # ax1.scatter([pt[1] for pt in border], [pt[0] for pt in border])

        # #ax.scatter([path_y], [path_x])
        # for x in koz:
        #     for pt in x:
        #         print(pt)
        #     ax1.scatter([pt[1] for pt in x], [pt[0] for pt in x])
        #     #plt.show(10)
